  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = var.enable_dlq ? aws_sqs_queue.lambda_dlq[0].arn : "*"
      }
    ]
  })
}
...
# API Gateway Integration (Lambda target resolved per stage via stage variable)
resource "aws_api_gateway_integration" "blue_lambda_integration" {
  ...
  type                    = "AWS_PROXY"
  uri                     = "arn:aws:apigateway:${data.aws_region.current.name}:lambda:path/2015-03-31/functions/arn:aws:lambda:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:function:$${stageVariables.lambda_function_name}/invocations"
}
...
resource "aws_api_gateway_stage" "production_stage" {
  ...
  variables = {
    lambda_function_name = local.blue_function_name
    environment          = "blue"
  }
  ...
  dynamic "canary_settings" {
    ...
    content {
      ...
      stage_variable_overrides = {
        lambda_function_name = local.green_function_name
        environment          = "canary"
      }
    }
  }
...
# Method-level settings for production stage
resource "aws_api_gateway_method_settings" "production_method_settings" {
  rest_api_id = aws_api_gateway_rest_api.advanced_deployment_api.id
...
# Method-level settings for staging stage
resource "aws_api_gateway_method_settings" "staging_method_settings" {
  rest_api_id = aws_api_gateway_rest_api.advanced_deployment_api.id
  stage_name  = aws_api_gateway_stage.staging_stage.stage_name
  method_path = "*/*"
